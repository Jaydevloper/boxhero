...
function handlerSubmit(e){
    e.preventDefault();
    const url = `http://localhost:5000/${token}/${searchRef.current.value}`;
     fetch(url)
     .then(res => res.json())
        ...
        <header className="site-header ">
                ...
                <path
                    ...
                    <div className="site-header__section container">
                        <div className="site-header__list">
                            <h2 className='site-header__head'>Product List</h2>
                            <form method="GET" action="#" onSubmit={handlerSubmit}>
                                <input className="site-header__search" type={"search"} placeholder="Search product name, barcode, category" ref={searchRef}/>
                            </form>
                        </div>
                        <div>
                            <Link className="site-header__link" to={'/add'}>+ Add Product</Link>
                            <Link className="site-header__data" to={'/data'}>Data Center</Link>
                        </div>
